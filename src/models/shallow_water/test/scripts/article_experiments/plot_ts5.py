...
for resolution in resolutions:#[32,48,64,96]:
    N = int(resolution[1:4])
    Nlon = 4*N
    Nlat = 2*N+1
    ...
    for scheme in schemes:
        print(N, scheme)
        wks = Ngl.open_wks(wktype, "ts5_h_"+resolution+"_"+scheme)
        Ngl.define_colormap(wks,"rainbow+white")
        fd = open(path+"/h_"+resolution+"_"+scheme+".dat","rb")
        fd.seek(4*Nlon*Nlat*15,0)
        h15 = np.fromfile(fd,count=Nlon*Nlat,dtype=np.float32).reshape(Nlat,Nlon)
        fd.close()
        fd = open(path+"/curl_"+resolution+"_"+scheme+".dat","rb")
        fd.seek(4*Nlon*Nlat*15,0)

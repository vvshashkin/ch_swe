...
public :: oscillating_gaussian_hill_scalar_field_generator_t
...
type, extends(scalar_field_generator_t) :: gaussian_hill_scalar_field_generator_t
    real(kind=8) :: phi0 = 0.0_8, lam0 = 0.0_8, d0 = 0.25_8
contains
    procedure :: get_scalar_field => generate_gaussian_hill_scalar_field
end type gaussian_hill_scalar_field_generator_t

type, extends(scalar_field_generator_t) :: oscillating_gaussian_hill_scalar_field_generator_t
    integer(kind=4) :: nu = 32 ! modulation wave-vector magnitude
contains
    procedure :: get_scalar_field => generate_oscillating_gaussian_hill_scalar_field
end type oscillating_gaussian_hill_scalar_field_generator_t
...
subroutine generate_gaussian_hill_scalar_field(this, f, npts, nlev, x, y, z)
    ...
    integer(kind=4) :: i, k
    real(kind=8)    :: x0, y0, z0, r

    x0 = cos(this%phi0)*cos(this%lam0)
    y0 = cos(this%phi0)*sin(this%lam0)
    z0 = sin(this%phi0)

    do k = 1, nlev
        do i=1, npts
            r = acos(min(1.0_8,max(-1.0_8,x(i)*x0+y(i)*y0+z(i)*z0)))
            f(i,k) = exp(-(r/this%d0)**2)
        end do
    end do
end subroutine generate_gaussian_hill_scalar_field

subroutine generate_oscillating_gaussian_hill_scalar_field(this, f, npts, nlev, x, y, z)

    use const_mod,      only : pi
    use sph_coords_mod, only : cart2sph

    class(oscillating_gaussian_hill_scalar_field_generator_t), &
                                    intent(in)  :: this
    integer(kind=4),                intent(in)  :: npts, nlev
    real(kind=8),                   intent(in)  :: x(npts), y(npts), z(npts)
    real(kind=8),                   intent(out) :: f(npts,nlev)
    ...
    integer(kind=4) :: i, k
    real(kind=8)    :: r, lam, phi

    do k = 1, nlev
        do i=1, npts
            call cart2sph(x(i), y(i), z(i), lam, phi)
            r = acos(cos(phi)*cos(lam-pi))
            f(i,k) = exp(-(4.0_8*r)**2)*(cos(this%nu*lam)*cos(this%nu*phi))**2
        end do
    end do
end subroutine generate_oscillating_gaussian_hill_scalar_field
...
subroutine generate_ts2_height_field(this, f, npts, nlev, x, y, z)
    ...
    integer(kind=4) :: i, k
    real(kind=8) :: lam, phi, v_lam, v_phi, xn(npts), yn(npts), zn(npts)
    real(kind=8) :: alpha, u0, h_mean, a, omega, grav, axis(3), metric_term
    ...
    metric_term = 1.0_8
    if(this%linear_mode) metric_term = 0.0_8
    ...
    do k = 1, nlev
        do i=1, npts
            ...
            f(i,k) = h_mean - (a*omega*u0+metric_term*0.5_8*u0**2)/grav*sin(phi)**2
        end do
    end do

...
subroutine create_vector_advection_operator(vec_advection_op, vec_advection_op_name, domain)
    ...
    case("vector_advection_C_c4")
        call create_vector_advection_C(vec_advection_op, v_nabla_c4_operator_t(), &
                                       "interp2d_uv2pvec_C_sbp42", "interp2d_pvec2uv_C_sbp42", &
                                                                                     3,domain)
    case("vector_advection_Ch_up4_cov")
        call create_vector_advection_Ch(vec_advection_op, v_nabla_up4_operator_t(), &
                                       "interp2d_uv2pvec_Ch_sbp42", "interp2d_pvec2uv_Ch_sbp42", &
                                                                              3,domain)
    case("vector_advection_Ch_up5_cov")
        call create_vector_advection_Ch(vec_advection_op, v_nabla_up5_operator_t(), &
                                       "interp2d_uv2pvec_Ch_sbp63", "interp2d_pvec2uv_Ch_sbp63", &
                                                                              3,domain)
    case default
...
subroutine create_vector_advection_C(vec_advection_op, v_nabla_op, uv2p_interp_name, &
    ...
    call create_grid_field(vec_advection_C_op%u_halo,8, 0, domain%mesh_u)
    call create_grid_field(vec_advection_C_op%v_halo,8, 0, domain%mesh_v)
...
subroutine create_vector_advection_Ch(vec_advection_op, v_nabla_op, uv2p_interp_name, &
                                      p2uv_interp_name, halo_width, domain)

    use vector_advection_Ch_mod,      only : vector_advection_Ch_t
    use grid_field_factory_mod,       only : create_grid_field
    use interpolator2d_factory_mod,   only : create_vec2vec_interpolator2d
    use halo_factory_mod,             only : create_vector_halo_procedure
    use abstract_v_nabla_mod,         only : v_nabla_operator_t
    ...
    type(vector_advection_Ch_t), allocatable :: vec_advection_Ch_op

    allocate(vec_advection_Ch_op)

    vec_advection_Ch_op%v_nabla_op = v_nabla_op

    call create_grid_field(vec_advection_Ch_op%u_at_v, 0, 0, domain%mesh_v)
    call create_grid_field(vec_advection_Ch_op%v_at_u, 0, 0, domain%mesh_u)
    call create_grid_field(vec_advection_Ch_op%uh,halo_width+2, 0, domain%mesh_p)
    call create_grid_field(vec_advection_Ch_op%vh,halo_width+2, 0, domain%mesh_p)
    call create_grid_field(vec_advection_Ch_op%u_halo,8, 0, domain%mesh_u)
    call create_grid_field(vec_advection_Ch_op%v_halo,8, 0, domain%mesh_v)

    call create_vec2vec_interpolator2d(vec_advection_Ch_op%interp_v2h_op, uv2p_interp_name, domain)
    call create_vec2vec_interpolator2d(vec_advection_Ch_op%interp_h2v_op, p2uv_interp_name, domain)

    call create_vector_halo_procedure(vec_advection_Ch_op%halo_uv, domain,max(halo_width,2),"ecs_Ch_vec")
    ! call create_vector_halo_procedure(vec_advection_Ch_op%tendency_edge_sync, domain, &
    !                                              1,"C_vec_default")
    call create_hor_Christofel(vec_advection_Ch_op%hor_Christofel,domain%mesh_u, &
                               domain%mesh_v,domain%metric,"covariant")


    vec_advection_Ch_op%CH_uv_exchange = &
        create_symmetric_halo_vec_exchange_Ch(domain%partition, domain%parcomm, &
                                              domain%topology, 1, "cross")

    call move_alloc(vec_advection_Ch_op, vec_advection_op)

end subroutine create_vector_advection_Ch

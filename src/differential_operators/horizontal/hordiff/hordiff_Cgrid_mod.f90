...
subroutine calc_diff_vec_div_part(this, u_tend, v_tend, u, v, domain)
    ...
    if(this%is_covariant) then
        call this%co2contra_op%transform(this%ut, this%vt, u, v, domain)
    else
        call this%ut%assign(u,domain%mesh_u)
        call this%vt%assign(v,domain%mesh_v)
    end if
    ...
    if(.not. this%is_covariant) then
        call this%co2contra_op%transform(u_tend, v_tend, this%ut, this%vt, domain)
    else
        call u_tend%assign(this%ut,domain%mesh_u)
        call v_tend%assign(this%vt,domain%mesh_v)
    end if
...
subroutine calc_diff_vec_curl_part(this, u_tend, v_tend, u, v, domain)
    ...
    use vec_math_mod, only : multiply_by_Jvert_self
    ...
    if(this%is_covariant) then
        ...
        call this%co2contra_op%transform2co(u_tend, v_tend, u, v, domain)
        call this%curl_op%calc_curl(this%curl, u_tend, v_tend, domain)
    end if

    call multiply_by_Jvert_self(this%curl,domain%mesh_q)

    do p = 2, this%diff_order

        call this%grad_perp_op%calc_grad_perp(this%ut, this%vt, this%curl, domain)
        call multiply_by_Jvert_self(this%ut,domain%mesh_u)
        call multiply_by_Jvert_self(this%vt,domain%mesh_v)

...
type, extends(laplace_Ah_sbp21_narrow_t) :: laplace_Ah_sbp42_narrow_t
    !Inherited from laplace_Ah_sbp21_narrow_t:
...
subroutine calc_laplace(this, f1, f, domain)
    ...
    do t = domain%partition%ts, domain%partition%te
        ...
        call calc_tile_d1x_d1y(this%d1f_x%tile(t), this%d1f_y%tile(t), f%tile(t), this%sbp_d1_op, this%mesh%tile(t))
    ...
    do t = domain%partition%ts, domain%partition%te
        ...
        call calc_laplace_tile(f1%tile(t), this%d2f_x%tile(t), this%d2f_y%tile(t), &
                                ...
                                f%tile(t), this%sbp_d1_op, this%mesh%tile(t), this%mesh%scale)
                                ! domain%mesh_xy%tile(t), domain%mesh_xy%scale)

    end do

    ! call divide_by_J_self(f1, domain%mesh_xy)
    call divide_by_J_self(f1, this%mesh)
...
subroutine calc_tile_d1x_d1y(d1f_x, d1f_y, f, sbp_d1_op, mesh_xy)
    ...
    class(sbp_diff_t),  intent(in) :: sbp_d1_op
    type(tile_mesh_t),  intent(in) :: mesh_xy

    integer(kind=4) :: i, j, k

    call sbp_d1_op%apply_tile(d1f_x, f, mesh_xy, "x")
    call sbp_d1_op%apply_tile(d1f_y, f, mesh_xy, "y")
...
subroutine calc_laplace_tile(fout,d2f_x, d2f_y, d2f_xy, d2f_yx, d1f_x, d1f_y, f, sbp_d1_op, mesh_xy, scale)
    ...
    call sbp_d1_op%apply_tile(d2f_yx, d1f_x, mesh_xy, "y")
    call sbp_d1_op%apply_tile(d2f_xy, d1f_y, mesh_xy, "x")

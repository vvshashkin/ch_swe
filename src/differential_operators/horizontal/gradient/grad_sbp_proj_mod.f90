...
type, public, extends(grad_operator_t) :: grad_sbp_proj_t
    class(exchange_t), allocatable :: exch_halo
    class(sbp_diff_t), allocatable :: sbp_diff
    class(halo_vec_t), allocatable :: proj_op
    type(mesh_t),      pointer     :: mesh_u, mesh_v, mesh_p
contains
    procedure, public :: calc_grad => calc_grad_proj_sbp
end type grad_sbp_proj_t

contains

subroutine calc_grad_proj_sbp(this, gx, gy, f, domain)
    class(grad_sbp_proj_t), intent(inout) :: this
    ...
    call this%proj_op%get_halo_vector(gx, gy, domain, 0)

end subroutine calc_grad_proj_sbp

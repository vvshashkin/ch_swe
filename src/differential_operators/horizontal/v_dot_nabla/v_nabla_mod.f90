...
type, public, extends(v_nabla_up4_operator_t) :: v_nabla_up5_operator_t
contains
    procedure, private, nopass :: calc_v_nabla_tile => calc_v_nabla_up5_tile
end type v_nabla_up5_operator_t
...
subroutine calc_v_nabla_up5_tile(f_tend, f, ut, vt, mesh, scale)
    ...
    real(kind=8), parameter :: c1 = -1.0_8/20.0_8, c2 = 0.5_8, c3 = 1.0_8/3.0_8, &
                               c4 = -1.0_8, c5 = 0.25_8, c6 = -1.0_8/30.0_8
    ...
    do k = ks, ke
        do j = js, je
            do i = is, ie
                ...
                zr = 1._8-zl
                dx = (zl*(c1*f%p(i+2,j,k)+c2*f%p(i+1,j,k)+c3*f%p(i,j,k)+ &
                          c4*f%p(i-1,j,k)+c5*f%p(i-2,j,k)+c6*f%p(i-3,j,k))-&
                      zr*(c1*f%p(i-2,j,k)+c2*f%p(i-1,j,k)+c3*f%p(i,j,k)+ &
                          c4*f%p(i+1,j,k)+c5*f%p(i+2,j,k)+c6*f%p(i+3,j,k)))

                zl = .5_8+sign(.5_8,vt%p(i,j,k))
                zr = 1._8-zl
                dy = (zl*(c1*f%p(i,j+2,k)+c2*f%p(i,j+1,k)+c3*f%p(i,j,k)+ &
                          c4*f%p(i,j-1,k)+c5*f%p(i,j-2,k)+c6*f%p(i,j-3,k))-&
                      zr*(c1*f%p(i,j-2,k)+c2*f%p(i,j-1,k)+c3*f%p(i,j,k)+ &
                          c4*f%p(i,j+1,k)+c5*f%p(i,j+2,k)+c6*f%p(i,j+3,k)))
...
end subroutine calc_v_nabla_up5_tile

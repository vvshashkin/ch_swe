...
type, extends(massflux_operator_t), public :: massflux_c_sbp_t

    class(interpolator2d_vec2vec_t), allocatable :: interp_f2uv_op

    contains

    procedure :: calc_massflux => calc_c_sbp_massflux
...
subroutine calc_c_sbp_massflux(this, fx, fy, f, u, v, domain)

    class(massflux_c_sbp_t), intent(inout) :: this
    type(domain_t),          intent(in)    :: domain
    type(grid_field_t),      intent(inout) :: f, u, v
    !output:
    type(grid_field_t),      intent(inout) :: fx, fy
    ...
    call fx%assign_prod(1.0_8, fx, u, domain%mesh_x)
    call fy%assign_prod(1.0_8, fy, v, domain%mesh_y)

end subroutine calc_c_sbp_massflux
...
!                 fx%p(mesh_o%nx+1,j,k) = u%p(mesh_o%nx+1,j,k)*f%p(mesh_o%nx,j,k)
!             end if
!         end do

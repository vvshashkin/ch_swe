...
type, extends(scalar_advection3d_t) :: advection_p_Ah3d_t
    class(vertical_operator_t), allocatable      :: interp_w2p_op
    integer(kind=4)                              :: halo_width_interior
    class(exchange_t), allocatable               :: exchange_p_interior
    integer(kind=4)                              :: halo_width
    class(halo_t), allocatable                   :: p_edge_sync
    class(v_nabla_operator_t), allocatable       :: v_nabla_op
    class(adv_z_t), allocatable                  :: adv_z
    type(grid_field_t)                           :: eta_dot_p, f_tend_z
    contains
    procedure calc_adv3d => calc_adv3d_Ah
end type advection_p_Ah3d_t
...
subroutine calc_adv3d_Ah(this,f_tend,f,u,v,eta_dot,domain)
    class(advection_p_Ah3d_t),   intent(inout) :: this
    type(grid_field_t),          intent(inout) :: f, u, v, eta_dot
    type(domain_t),              intent(in)    :: domain
    !output
    type(grid_field_t),          intent(inout) :: f_tend
    ...
    call this%interp_w2p_op%apply(this%eta_dot_p, eta_dot, domain)
    call this%exchange_p_interior%do(f,domain%parcomm)

    call this%v_nabla_op%calc_v_nabla(f_tend, f, u, v, domain%mesh_p)
    call this%adv_z%calc_z_adv(this%f_tend_z,f,this%eta_dot_p,domain%mesh_p)

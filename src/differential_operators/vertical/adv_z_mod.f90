...
type, extends(adv_z_t) :: adv_z_up_mix4_t
    contains
    procedure :: calc_z_adv_tile => calc_z_adv_up_mix4_tile
end type adv_z_up_mix4_t
...
subroutine calc_z_adv_up_mix4_tile(this, f_tend, f, eta_dot, mesh,scale)
    class(adv_z_up_mix4_t), intent(in)    :: this
    type(tile_field_t),     intent(in)    :: f, eta_dot
    type(tile_mesh_t),      intent(in)    :: mesh
    real(kind=8),           intent(in)    :: scale
    !output
    ...
    do k = max(ks,1), min(ke,2)
        km1 = max(1,k-1)
        do j=js, je
            do i=is, ie
                left  = 0.5_8+sign(0.5_8,eta_dot%p(i,j,k))
                right = 1.0_8-left
                df = (right*(f%p(i,j,k+1)-f%p(i,j,k)) + &
                      left *(f%p(i,j,k)-f%p(i,j,km1))) / (mesh%hz*scale)
                f_tend%p(i,j,k) =-eta_dot%p(i,j,k)*df
            end do
        end do
    end do
    ...
    do j=js, je
        do i=is, ie
            left  = 0.5_8+sign(0.5_8,eta_dot%p(i,j,k))
            right = 1.0_8-left
            df = (left *(2._8*f%p(i,j,k+1)+3._8*f%p(i,j,k)-6._8*f%p(i,j,k-1)+f%p(i,j,k-2)) -&
                  right*(2._8*f%p(i,j,k-1)+3._8*f%p(i,j,k)-6._8*f%p(i,j,k+1)+f%p(i,j,k+2))) &
                                                               / (6._8*mesh%hz*scale)
            f_tend%p(i,j,k) =-eta_dot%p(i,j,k)*df
        end do
    end do
    do k=max(ks,4), min(ke,mesh%nz-3)
        !constant extension of field above/below the the upper/lower boundary
        do j=js, je
            do i=is, ie
                left  = 0.5_8+sign(0.5_8,eta_dot%p(i,j,k))
                right = 1.0_8-left
                df =  (left*( 3._8*f%p(i,j,k+1)+10._8*f%p(i,j,k)- &
                             18._8*f%p(i,j,k-1)+6._8*f%p(i,j,k-2)-f%p(i,j,k-3))-&
                      right*( 3._8*f%p(i,j,k-1)+10._8*f%p(i,j,k)- &
                             18._8*f%p(i,j,k+1)+6._8*f%p(i,j,k+2)-f%p(i,j,k+3))) &
                                                         / (12.0_8*mesh%hz*scale)
                f_tend%p(i,j,k) =-eta_dot%p(i,j,k)*df
            end do
    ...
    do k = max(ks,mesh%nz-1), min(ke,mesh%nz)
        kp1 = min(k+1,mesh%nz)
        do j=js, je
            do i=is, ie
                left  = 0.5_8+sign(0.5_8,eta_dot%p(i,j,k))
                right = 1.0_8-left
                df = (left *(f%p(i,j,k)-f%p(i,j,k-1)) + &
                      right*(f%p(i,j,kp1)-f%p(i,j,k))) / (mesh%hz*scale)
                f_tend%p(i,j,k) =-eta_dot%p(i,j,k)*df
            end do
        end do
    end do
end subroutine calc_z_adv_up_mix4_tile

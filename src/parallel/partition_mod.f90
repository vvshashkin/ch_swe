...
use smart_array_mod, only : array_container_i4_2d

implicit none

type, public :: partition_t
    type(tiles_t) :: tiles_o, tiles_x,  tiles_y,  tiles_xy,  &
                     tiles_z, tiles_xz, tiles_yz, tiles_xyz, &
    ...
    integer(kind=4)              :: ts, te      ! start and end index of the tiles belonging to the current processor
    integer(kind=4), allocatable :: ts_global(:), te_global(:) ! array of start and end index of the tiles belonging to the specific processor 

    type(array_container_i4_2d), allocatable :: tile_map_xy(:) !tile_map_xy(face)%a(i,j) to determine tile owning point with this indices
...
!We assume that number of tiles at each panel are the same
    ...
    allocate(this%ts_global(0:Np-1), this%te_global(0:Np-1))

    do ip = 0, Np-1
        this%ts_global(ip) = findloc(this%proc_map, ip, dim=1)
        this%te_global(ip) = findloc(this%proc_map, ip, back = .true., dim=1)
    end do
    ...
    call this%tiles_xz%init (Nt, Nz_inter, Nh,   Nh+1, this%tiles_o%tile)
    call this%tiles_yz%init (Nt, Nz_inter, Nh+1, Nh,   this%tiles_o%tile)
    ...
    do t = 1, Nt
        ...
        if (this%tiles_xz%tile(t)%ie == nh) this%tiles_xz%tile(t)%ie = nh+1
        if (this%tiles_yz%tile(t)%je == nh) this%tiles_yz%tile(t)%je = nh+1
        ...
        if(this%tiles_xz%tile(t)%ke == Nz) this%tiles_xz%tile(t)%ke = Nz_inter
        if(this%tiles_yz%tile(t)%ke == Nz) this%tiles_yz%tile(t)%ke = Nz_inter
    ...
    call make_tile_map(this%tile_map_xy, this%tiles_xy, this%panel_map, this%num_panels, this%Nt)

end subroutine init

subroutine make_tile_map(tile_map, tiles, panel_map, num_panels, num_tiles)

    type(array_container_i4_2d), allocatable, intent(out) :: tile_map(:)
    type(tiles_t),                            intent(in)  :: tiles
    integer(kind=4),                          intent(in)  :: panel_map(:)
    integer(kind=4),                          intent(in)  :: num_panels, num_tiles

    integer(kind=4) :: is, js, ie, je, i, t

    allocate(tile_map(num_panels))

    do i = 1, num_panels
        allocate(tile_map(i)%a(tiles%Nx,tiles%Ny))
        tile_map(i)%a = -999
    end do
    ...
    do t = 1, num_tiles
        ...
        tile_map(panel_map(t))%a(is:ie,js:je) = t
    end do

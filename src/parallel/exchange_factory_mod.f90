...
function create_o_z_points_halo_exchange(tiles, partition, parcomm, topology, halo_width, &
    ...
    integer(kind=4) :: Nt_loc, Nt_rem, it_loc, it_rem
    ...
    do local_ind = partition%ts, partition%te
        ...
        Nt_loc = partition%te - partition%ts + 1 ! number of tiles at local MPI process
        it_loc = local_ind - partition%ts        ! tile index at local MPI process (0 <= it_loc < Nt_loc)
        ...
        do remote_ind = 1, partition%Nt
            ...
            Nt_rem = partition%te_global(partition%proc_map(remote_ind)) &
                   - partition%ts_global(partition%proc_map(remote_ind)) + 1 ! number of tiles at remote MPI process
            it_rem = remote_ind - partition%ts_global(partition%proc_map(remote_ind)) ! tile index at remote MPI process (0 <= it_rem < Nt_rem)
            ...
            if (is_intersection) then
                ...
                recv_tag(rnum) = generate_tag(it_rem, it_loc, Nt_rem, Nt_loc)
            end if
            ...
            if (is_intersection) then
                ...
                send_tag(snum) = generate_tag(it_loc, it_rem, Nt_loc, Nt_rem)
            end if
        end do
    end do
...
function create_symmetric_halo_vec_exchange_Ch(partition, parcomm, topology, halo_width, halo_type) result(exchange)
    ...
    exchange%exch_u = create_symm_halo_vec_exchange_U_points(partition, parcomm, topology, halo_width, halo_type,partition%tiles_x,partition%tiles_y)
    exchange%exch_v = create_symm_halo_vec_exchange_V_points(partition, parcomm, topology, halo_width, halo_type,partition%tiles_x,partition%tiles_y)
...
function create_symmetric_halo_vec_exchange_Cz(partition, parcomm, topology, halo_width, halo_type) result(exchange)
    ...
    exchange%exch_u = create_symm_halo_vec_exchange_U_points(partition, parcomm, topology, halo_width, halo_type,partition%tiles_xz,partition%tiles_yz)
    exchange%exch_v = create_symm_halo_vec_exchange_V_points(partition, parcomm, topology, halo_width, halo_type,partition%tiles_xz,partition%tiles_yz)

end function create_symmetric_halo_vec_exchange_Cz

function create_symm_halo_vec_exchange_U_points(partition, parcomm, topology, halo_width, halo_type, tiles_x, tiles_y) result(exchange)
    ...
    character(*),     intent(in) :: halo_type
    type(parcomm_t),  intent(in) :: parcomm
    type(tiles_t),    intent(in), target :: tiles_x, tiles_y
    ...
    do local_ind = partition%ts, partition%te
        ...
        Nt_loc = partition%te - partition%ts + 1 ! number of local tiles at local MPI process
        it_loc = local_ind - partition%ts        ! tile index at local MPI process (0 <= it_loc < Nt_loc)
        ...
        do remote_ind = 1, partition%Nt
            ...
            !recv part
            local_tile => tiles_x%tile(local_ind)
            ...
            !recv from u points to u points
                remote_tile => tiles_x%tile(remote_ind)
            else if (first_dim_index(snum+1) == 'j') then
            !recv from v points to u points
                remote_tile => tiles_y%tile(remote_ind)
            end if

            call topology%transform_tile_coords(remote_panel, remote_tile,     &
                                                local_panel,  temp_tile,       &
                                                tiles_x%Nx, tiles_x%Ny)
            ...
            if (first_dim_index(snum+1) == 'i') then
                ...
                local_tile  => tiles_y%tile(local_ind)
                remote_tile => tiles_x%tile(remote_ind)
                call topology%transform_tile_coords(remote_panel, remote_tile, &
                                                    local_panel, temp_tile,     &
                                                    tiles_y%Nx, tiles_y%Ny)
...
function create_symm_halo_vec_exchange_V_points(partition, parcomm, topology, halo_width, halo_type, tiles_x, tiles_y) result(exchange)
...
!> This function generates a unique tag based on the sender and receiver tile indices
!! and the total number of tiles managed by each. The tag is used to identify
!! a specific communication exchange between two MPI processes in a parallel environment.
!!
!! @param[in] sender_tile_ind     The tile index of the sender MPI process.
!! @param[in] reciever_tile_ind   The tile index of the receiver MPI process.
!! @param[in] sender_tiles_num    The total number of tiles managed by sender MPI process.
!! @param[in] reciever_tiles_num  The total number of tiles managed by receiver MPI process.
!!
!! @return An integer tag that uniquely identifies the exchange between the sender
!! and receiver MPI processes.
function generate_tag(sender_tile_ind, reciever_tile_ind, sender_tiles_num, reciever_tiles_num) result(tag)

    integer(kind=4), intent(in) :: sender_tile_ind
    integer(kind=4), intent(in) :: reciever_tile_ind
    integer(kind=4), intent(in) :: sender_tiles_num
    integer(kind=4), intent(in) :: reciever_tiles_num

    integer(kind=4) :: tag

    tag = sender_tiles_num*reciever_tile_ind + sender_tile_ind

end function generate_tag

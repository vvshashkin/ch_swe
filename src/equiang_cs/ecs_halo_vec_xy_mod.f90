...
!non-staggered (Ah/xy) grid. The algorithm is as follows:
...
module ecs_halo_vec_xy_mod

use halo_mod,          only : halo_t, halo_vec_t
use exchange_halo_mod, only : exchange_t
use ecs_halo_xy_mod,   only : ecs_tile_halo_xy_t

implicit none

type, extends(halo_vec_t) :: ecs_halo_xy_vec_t

    integer(kind=4)                           :: ts, te
    class(exchange_t),            allocatable :: exch_halo
    type(ecs_tile_halo_xy_vec_t), allocatable :: tile(:)

    contains

    procedure :: get_halo_vector => get_ecs_xy_vector_halo

end type

type ecs_tile_halo_xy_vec_t
  ...
  class(ecs_tile_halo_xy_t), allocatable :: scalar_halo !scalar halo to interpolate individual
                                                        !vector components after transformation
  contains
  procedure, public :: interpv  => ext_ecs_xy_vector_tile_halo

end type ecs_tile_halo_xy_vec_t

contains

subroutine get_ecs_xy_vector_halo(this,u,v,domain,halo_width)
    use grid_field_mod, only : grid_field_t
    use domain_mod,     only : domain_t

    class(ecs_halo_xy_vec_t), intent(inout) :: this
...
end subroutine get_ecs_xy_vector_halo

subroutine ext_ecs_xy_vector_tile_halo(this, u, v, tile, halo_width)
!interpolate source face vectors at halo zones to target face virtual points
use tile_mod,       only : tile_t
use grid_field_mod, only : tile_field_t

class(ecs_tile_halo_xy_vec_t), intent(in)    :: this
...
if(this%lhalo(1)) then
    do k = ks, ke
        do j=1,halo_width+1 !plus 1 to account for collocated row of adjacent panel
...
end subroutine ext_ecs_xy_vector_tile_halo

end module ecs_halo_vec_xy_mod

...
use metric_2d_ecs_mod,           only : ecs_b1_proto, ecs_b2_proto, ecs_a1_proto, ecs_a2_proto
...
subroutine create_ecs_Ah_vec_sync(halo_out,domain,halo_width,components_type)
    ...
    real(kind=8)    :: alpha, beta, a(3), b(3), v(3)
    ...
    do t=halo%ts,halo%te
        ...
        if(js == 1) then
            ...
            do i = is,ie
                ...
                v(1:3) = ecs_a2_proto(alpha, 0.25_8*pi)
                a(1:3) = [v(1),-v(3),v(2)]
        ...
        if(is == 1) then
            ...
            do j = js,je
                ...
                v(1:3) = ecs_a1_proto( 0.25_8*pi, beta)
                a(1:3) = [-v(3),v(2),v(1)]
    ...
    do t=domain%mesh_xy%ts, domain%mesh_xy%te
        ...
        if(js == 1) then
            ...
            do i = is,ie
                ...
                v(1:3) = ecs_b1_proto(alpha, 0.25_8*pi)
                b(1:3) = [v(1),-v(3),v(2)]
                ! b(1:3) = ecs_b1_proto(alpha, 0.25_8*pi)
                ! b(1:3) = [b(1),-b(3),b(2)]
                halo%tile(t)%qb(i) = sum(a(1:3)*b(1:3))
        ...
        if(is == 1) then
            ...
            do j = js,je
                ...
                v(1:3) = ecs_b2_proto( 0.25_8*pi, beta)
                b(1:3) = [-v(3),v(2),v(1)]
                ! b(1:3) = ecs_b2_proto( 0.25_8*pi, beta)
                ! b(1:3) = [-b(3),b(2),b(1)]
                halo%tile(t)%ql(j) = sum(a(1:3)*b(1:3))

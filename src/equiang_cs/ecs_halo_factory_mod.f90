...
public   :: create_ecs_o_scalar_halo, init_ecs_tile_halo, &
            create_ecs_xy_scalar_halo, init_ecs_tile_halo_xy
...
subroutine create_ecs_xy_scalar_halo(halo_out,domain,halo_width,is_z_interfaces)
    use halo_mod,             only : halo_t
    use domain_mod,           only : domain_t
    use exchange_factory_mod, only : create_xy_points_halo_exchange, create_xyz_points_halo_exchange
    ...
    if(.not. is_z_interfaces) then
        ...
        halo%exch_halo = create_xyz_points_halo_exchange(domain%partition, domain%parcomm, &
                                                         domain%topology, ex_halo_width, 'full')
    end if
